...
module.exports = function(bot, config) {
  bot.command('mediafire', async ctx => {
    try {
      const mfUrl = ctx.message.text.split(' ').slice(1).join(' ');
      const url = `https://api.princetechn.com/api/download/mediafire?apikey=prince&url=${encodeURIComponent(mfUrl)}`;
      ...
      await ctx.reply('Failed to fetch Mediafire result.');
    }
  });
};